...
describe("enterprise calibration", () => {
  it("keeps obvious enterprise stacks strongly on Enterprise", () => {
    ...
    expect(result.scores.mui_x_enterprise).toBeGreaterThan(
      result.scores.mui_x_premium
    );
    expect(
      result.scores.mui_x_enterprise - result.scores.mui_x_premium
  ...
  it("keeps enterprise-support-only moderate-scale cases off Enterprise", () => {
    ...
    expect(internalResult.scores.mui_x_premium).toBeGreaterThan(
      internalResult.scores.mui_x_enterprise
    );
  });
  ...
  it("keeps org-wide intent alone on Premium", () => {
    ...
    expect(result.scores.mui_x_premium).toBeGreaterThan(
      result.scores.mui_x_enterprise
    );
  });
  ...
  it("keeps large low-complexity scale cases on Premium with a clearer margin", () => {
    ...
    expect(
      result.scores.mui_x_premium - result.scores.mui_x_enterprise
    ).toBeGreaterThanOrEqual(2);
  });
});
